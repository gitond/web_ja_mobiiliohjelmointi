...
class App extends React.Component {
	...
	componentDidMount() {
		...
		axios
			.get(bURL + '/') // bURL ilman '/' loppua post requesteihin. Hiukan "Jank" ratkaisu I know, korjaa jos aikaa
			.then(response => {
	...
	addPerson = (event) => {
		...
		if (this.state.persons.map(person => person.name).includes(this.state.newName)){
			...
			axios.post(bURL, addPerson).then(() => {window.location.reload(true);})
			...
			//window.location.reload(true); // Uudelleenlataus jolloin lisätyn numeron backendgeneroitu id latautuu
		}
